...
//shared int it is shared between players and referee.
...
/*
thread_player() and thread_referee() are the main function of two types of thread. Compelete the functions so two
players can play the game.

Structure shared_int_t is used for players to send their choice to referee. The referee shares choice1 with player 1 and
choice2 with player 2. mutex and cond are used for synchronization.

Sturcture result_t is for referee to announce the outcome to players. The referee shares result with both players. barrier
...
/*
psuedo coding this assignemetn

player threads think for a bit about thiir choice
the thread_player function is ambiguous to threads

a thread will think for a bit
THINKGING()

A mutex will be locked on the choice

a player will make thiir choice- something like choice->value = r;
...
void * thread_player(void *arg_in)
{
    ...
    for (int i = 0; i < arg->n_rounds; i ++ ) {
        ...
        // now send choice to referee
        THINKING();
        pthread_mutex_lock(&choice->mutex);
        choice->value = r;
        choice->status = S_READY; //IM READY WOOHOO

        pthread_cond_broadcast(&choice->cond);
        pthread_mutex_unlock(&choice->mutex);

        pthread_barrier_wait(&result->barrier);

        outcome = result->value; //getting the reslt from the ref and saving it 
...
void * thread_referee(void *arg_ink)
{
    referee_arg_t *arg = arg_ink;
