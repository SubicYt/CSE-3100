...
/*
QUESTIONS:
1.) fork returns pid in the parent process. why?
2.) this assignment is fucking up my understanding so bad, im so cooked
3.) pipes from child to child?
4.) pipes from parent to child?
5.) what role does the parent have in this?
6.) what is the parent doing?
7.) When to close for a dup2?
8.) when should i be closing unused FDs in the start_program function?????
9.) WHY DOES THIS NOT WORK dup2(programs[cur + 1].fd_in, STDIN_FILENO);
10.) Explain wait to me. and wait pid
11.) explain execvp(FILE*, argv)
*/
...
{
    //start with setting up the pipes
    int fds[2]; //i think only 1 pipe is necessary
    //only need a pipe for everything that isnt the last row
    if(cur < num_programs - 1){
        if(pipe(fds) < 0){
            die("pipe failed");
            exit(EXIT_FAILURE);
        }
        programs[cur].fd_out = fds[PIPEFD_WRITE];
        programs[cur + 1].fd_in = fds[PIPEFD_READ];
    }
