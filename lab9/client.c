...
int main(int argc, char *argv[])
{
    ...
    while(result!=0){
        ...
        status = recv_lines(sockfd, buf, LINE_SIZE);
        if (status == -1) {
            close(sockfd);
            die("receive");
        }
        ...
        if (newline != NULL) {
            *newline = '\0';
            status = get_number(buf, &result);
            if (status == -1) {
                close(sockfd);
                die("failed to get number from buffer");
            }
            printf("%d\n", result);
