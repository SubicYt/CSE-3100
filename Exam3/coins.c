...
void *counter(void *t) {
  ...
  while (!done) {
    ...
    pthread_mutex_lock(&pdata->mutex);
      (*p_count)++;
      pdata->ready = 0;
    pthread_cond_broadcast(&pdata->cond_toss);
    pthread_mutex_unlock(&pdata->mutex);
    }  

    if (!pdata->ready && pdata->n == 0) {
      done = 1; //break from loop here
      pthread_cond_wait(&pdata->cond[id], &pdata->mutex);
    }
...
void *tosser(void *t) {
  ...
  while (!done) {
    ...
    pdata->data = c; //current toss outcome in decimal value
    pdata->ready = 1; //ready
    pdata->n = pdata->n - 1; //number of tosses i still need to produce.
    pthread_cond_broadcast(&pdata->cond[id]); 
    pthread_mutex_unlock(&pdata->mutex);
    ...
    if (pdata->n == 0)
      ...
      done = 1; //break from the loop here. 
