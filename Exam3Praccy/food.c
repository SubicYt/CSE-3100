...
void add_to_buffer(int item, int col, two_d_buffer *p)
{
	...
	p->buf[counts[col]][col] = item;
	p->remain--; //remaining items 
...
void remove_from_buffer(int *a, int *b, int *c, two_d_buffer *p)
{
	...
	//slide the rest of the items in each respective column back to their original positions.
	for(int i = 1; i < p->counts[0]; i++){
		p->buf[i-1][0] = p->buf[i][0]
	}

	for(int j = 1; j < p->counts[1]; j++){
		p->buf[j-1][1] = p->buf[j][1];
	}
	
	for(int k = 1; k < p->counts[2]; k++){
		p->buf[k-1][2] = p->buf[k][2];
	}
