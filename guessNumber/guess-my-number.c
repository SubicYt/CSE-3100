...
#define MAX_VALUE 1000 //max number
#define MSG_BUF_SIZE 100 //size of message
#define PIPEFD_READ 0
#define PIPEFD_WRITE 1

typedef  struct {
    int v; //value to guess?
    int num_attempts; //number of guesses made
    char message[MSG_BUF_SIZE]; 
    //The final message includes the number of
    //attempts the parent has made to guess the correct number.
} gmn_t;
...
void gmn_init(gmn_t *pg, int seed)
{
    ...
    pg->num_attempts = 0; //attempts starts at zero. 
...
int gmn_check(gmn_t *pg, int guess)
{
    ...
    if (pg->v == guess) {
        ...
        //NOTE: snprintf(string, size, format)
        return 0;
    }
    if (pg->v > guess)
        return 1; //greater than guess return 1
    return -1; //less than guess return -1 
}
...
void guess_my_number(int seed)
{
    ...
    //NOTE: fputs(const char* string, FILE* strem) outputs to file. 
}
...
void child_main(int fdp[], int fdc[], int seed)
{
    ...
    close(fdc[PIPEFD_READ]); // child does not read from fdc
    close(fdp[PIPEFD_WRITE]); // child does not write to fdp
    write_to(max, fdc[PIPEFD_WRITE]); // send max to parent
    //do while loop is useful here.
    do {
        read(fdp[PIPEFD_READ], &guess, sizeof(int)); //wait for guess from parent
        //read data is being stored within the guess variable
        result = gmn_check(&gmn, guess);
        write_to(result, fdc[PIPEFD_WRITE]); //write the result back to the parent
    } while(result != 0); //exit once number was correctly guessed. 
...
int main(int argc, char *argv[])
{
    ...
    read(fdc[PIPEFD_READ], &max, sizeof(int)); //reads the max;
    do { 
        guess = (min + max)/2;
        printf("My guess: %d\n", guess);
